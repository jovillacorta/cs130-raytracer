    ...
    const vec3& normal,int recursion_depth) const
{
    ...
    //first find if in shadow
    	...
    	Ray sRay(intersection_point,lvec); //create a shadow ray from intersect towards the light
		Hit shadowHit = world.Closest_Intersection(sRay); //check: does the shadow ray hit anything?

	    if (!shadowHit.object || lvec.magnitude() < shadowHit.dist || !world.enable_shadows){ 
	    //if object is not in shadow of another object, or if intersected object is behind light, 
	    //then for each light find their diffuse and specular intensities
